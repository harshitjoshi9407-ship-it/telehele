...
const checkFileExist = (message, outputFolder) => {
  ...
  // Use the EXACT same logic as getMediaPath to ensure consistency
  const expectedPath = getMediaPath(message, outputFolder);
  
  // Only return true if the file actually exists AND has content
  if (fs.existsSync(expectedPath)) {
    try {
      const stats = fs.statSync(expectedPath);
      // File must exist AND have content (not empty/corrupted)
      return stats.size > 0;
    } catch (error) {
      // If we can't read the file stats, consider it as non-existent
      return false;
    }
  }
